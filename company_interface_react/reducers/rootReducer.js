    ...
    switch (action.type) {
        ...
        * @param action.responses - ответы от /api/entrances/?house_id=house_id
        ...
        case 'SET_HOUSE_BALANCES': {
            ...
            Object.keys(action.response).map(room_id => {
                let room_balances = action.response[room_id],
                    room = rooms[room_id] || null; // квартира, к которой привязан баланс

                if (!room) return;

                Object.keys(room_balances).map(account_balances => {
                    Object.keys(room_balances[account_balances])
                        .filter(account_balance => room_balances[account_balances][account_balance].supplier_id === null) // убираем услуги по прямым договорам
                        .map(account_balance => {
                            let balance = room_balances[account_balances][account_balance]
                            let service = state.services[balance.service_id];
                            // let serviceType = service && state.serviceTypes[service.service_type_id];
                            ...
                            let service_type = state.services[balance.service_id].service_type_id
                            balancesMap.accounts[balance.account_id].all += +balance.amount;
                            if(!balancesMap.accounts[balance.account_id].service_types[service_type]){
                                balancesMap.accounts[balance.account_id].service_types[service_type]={
                                    amount:0,
                                    bill_entry_type_id:null
                                }
                            }

                            balancesMap.accounts[balance.account_id].service_types[service_type].amount += +balance.amount;
                            balancesMap.accounts[balance.account_id].service_types[service_type].bill_entry_type_id = balance.bill_entry_type_id
                            ...
                            //     sub_services: {}
                            // }
                            // balancesMap.houses[room.house_id] = balancesMap.houses[room.house_id] || {
                            //     all: 0,
                            //     resources: 0
                            // }


                            // balancesMap.rooms[room.room_id].all += +balance.amount; Квартиры тоже
                            // balancesMap.houses[room.house_id].all += +balance.amount; Дома не нужны
        ...
        case 'SET_SERVICES': {
            ...
            Object.keys(services)
                .filter(service => service.supplier_id === null)
                .map(service_id => {
