            ...
            responses.map( response =>  {
                if( response.success && response.data ){
                    response.data.map( room => {
                        if( room.room_id ){ rooms[room.room_id] = room; }
                    })
                }
            });
            return rooms;
...
export function fetchRoomDialogData( dispatch, room_id ){
    return  Promise.resolve()
        ...
        .then( services => dispatch({ type: 'SET_ROOM_SERVICE_VALUES', room_id, services }))
...
export function fetchAllPersons(){
    return Promise.all([fetchData('people'),fetchData('entities')])
        .then( responses => ({  persons: responses[0].success && responses[0].data || [],
