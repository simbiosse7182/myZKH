    ...
    render() {
        ...
        return (
            <div>
                ...
                <div className="row"><ButtonPanel onScroll={() => console.log('scrolled!')}> button panel </ButtonPanel>
                </div>
    ...
    return (
        <div className="panel panel-default">
            ...
            <table className="table">
                ...
                {!contacts
                    ? <tr>
                        <td> Нет проживающих или зарегистрированных</td>
                    </tr>
                            ...
                            <tr key={index}>
                                <td className="has-icon relative" onClick={() => toggleContact(contact.id)}>
                                    ...
                                    <div className="pull-badge">
                                        {contact.account ?
                                            <span className="label label-success">Лицевой Счёт</span> : null}{` `}
                                        {contact.user ? <span className="label label-success">Доступ</span> : null}{` `}
                                        {contact.owner ?
                                            <span className="label label-primary">Собственник</span> : null}{` `}
                                        {contact.resident ?
                                            <span className="label label-primary">Прописан</span> : null}{` `}
        ...
        <tr>
            <td className="td-form" style={{paddingLeft: '28px'}}>
                <table className="table-striped">
                    ...
                    {person && person.phone
                        ? <tr>
                            <td>Телефон: <span className="pull-right">{person.phone}</span></td>
                        </tr>
                        : null
                    }
                    {person && person.email
                        ? <tr>
                            <td>Email: <span className="pull-right">{person.email}</span></td>
                        </tr>
                        : null
                    }
                    {person && person.birth_date
                        ? <tr>
                            <td>Дата рождения: <span
                                className="pull-right">{dateFormat(Date.parse(person.birth_date), 'dd.mm.yyyy')}</span>
                            </td>
                        </tr>
                        : null
                    }
                    {entity && entity.inn
                        ? <tr>
                            <td>ИНН: <span className="pull-right">{entity.inn}</span></td>
                        </tr>
                        : null
                    }
                    {entity && entity.ogrn
                        ? <tr>
                            <td>ОГРН: <span className="pull-right">{entity.ogrn}</span></td>
                        </tr>
                        : null
                    }
                    {entity && entity.kpp
                        ? <tr>
                            <td>КПП: <span className="pull-right">{entity.kpp}</span></td>
                        </tr>
                        : null
                    }
                    </tbody>
        ...
        <div className="panel panel-default">
            <div className="panel-heading relative">
                {balances && balances.all < 0
                    ? <span className="pull-right badge badge-heading badge-danger">Долг: {balances.all.toFixed(2) * -1}
                        <span className="rub">a</span></span>
                    : <span
                        className="pull-right badge badge-heading badge-success">Переплата: {balances ? balances.all.toFixed(2) : 0}
                        <span className="rub">a</span></span>
                }
                ...
                <table className="table table-bordered">
                    ...
                    {Object.keys(balances.services).map(key => (
                        service_types[key] && service_types[key].name
                            ? <tr key={key}>
                                ...
                                <td className={`text-right ${ +balances.services[key] >= 0 ? 'text-success' : 'text-danger'}`}
                    ...
                    {Object.keys(balances.sub_services).map(key => (
                        service_types[key] && service_types[key].name
                            ? <tr key={key}>
                                ...
                                <td className={`text-right ${ +balances.sub_services[key] >= 0 ? 'text-success' : 'text-danger'}`}
                                    style={{verticalAlign: 'top'}}>
                    ...
                    {Object.keys(balances.sub_services).length && balances.services['null']
                        ? <tr>
                            ...
                            <td className={`text-right ${ +balances.services[null] >= 0 ? 'text-success' : 'text-danger'}`}
                                style={{verticalAlign: 'top'}}>
                    ...
                    <tr className="tr-summary text-bold">
                        ...
                        <td className={`text-right ${ +balances.all >= 0 ? 'text-success' : 'text-danger'}`}
                            style={{verticalAlign: 'top'}}>
