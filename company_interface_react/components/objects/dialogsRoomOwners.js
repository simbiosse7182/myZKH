        ...
        <div>
            <div className="panel panel-default" style={{marginBottom: '0px'}}>
                <div className="panel-heading panel-toggle-sm">
                    <h4 className="panel-title">Собственники <span
                        className="badge">{props.data.room.owners.length}</span></h4>
                    <div className="pull-right">
                        {!props.isEditing
                            ? <button type="button" className="btn btn-icon btn-primary btn-raised btn-edit pull-right"
                                      onClick={() => props.enableEditor('owners')}>Редактировать</button>
                            : <button type="button" className="btn btn-default btn-clear btn-icon" title="Отменить"
                                      onClick={() => props.disableEditor('owners')}>Закрыть</button>
                        }
                    </div>
                </div>
            </div>
...
class RoomOwnersRow extends React.Component {
    state = {
    ...
    componentDidMount() {
        ...
        const email = this.props.data.owners[this.props.owner_id] && this.props.data.owners[this.props.owner_id].person
            ? this.props.data.owners[this.props.owner_id].person.email
            : this.props.data.owners[this.props.owner_id] && this.props.data.owners[this.props.owner_id].entity
                ? this.props.data.owners[this.props.owner_id].entity.email || this.props.data.owners[this.props.owner_id].entity.email
                : ' '
        const phone = this.props.data.owners[this.props.owner_id] && this.props.data.owners[this.props.owner_id].person
            ? this.props.data.owners[this.props.owner_id].person.phone
            : this.props.data.owners[this.props.owner_id] && this.props.data.owners[this.props.owner_id].entity
                ? this.props.data.owners[this.props.owner_id].entity.email || this.props.data.owners[this.props.owner_id].entity.phone
                : ' '
    ...
    render() {
        const createUser = () => {
            ...
            return fetchData('users', {method: "POST", data})
        ...
        const updateUser = () => {
            ...
            return fetchData("users/" + this.props.data.owners[this.props.owner_id].user_id, {method: "put", data})
                .then(res => console.log(res))
        }
        const props = this.props
        const name = props.data.owners[props.owner_id] && props.data.owners[props.owner_id].person
        ...
        return ([
                <tr key={props.owner_id}>
                    ...
                    <td className="td-btn">
                        <button
                            data-toggle="modal" data-target={`#${props.owner_id + "modal"}`} aria-expanded="false"
                            aria-controls="collapseExample"
                            className={`btn btn-primary btn-icon-sm btn-${props.data.owners[props.owner_id].user_id ? 'refresh' : 'add'}`}
                            type="button"/>
                    </td>
                </tr>,

                <div
                    key={props.owner_id + "modal"}
                    className="modal fade"
                    tabIndex="-1" role="dialog"
                    id={props.owner_id + "modal"}>
                            ...
                            <div className="modal-body">
                                ...
                                <div className="form-group">
                                    <div className="input-group">
                                        <input className="form-control au-target"
                                               type="email"
                                        ...
                                        {!this.state.editEmail ?
                                            ...
                                            <button
                                                ...
                                                type="button"
                                                className="btn btn-primary btn-icon-sm btn-edit au-target"
                                ...
                                <div className="form-group">
                                    <div className="input-group">
                                                <span className="input-group-addon"
                                                      id={props.owner_id + "phone"}>+7</span>
                                        <input
                                            aria-describedby={props.owner_id + "phone"}
                                        ...
                                        {!this.state.editPhone ?
                                            <span className="input-group-btn">
                                            <button onClick={() => this.setState({editPhone: true})}
                                                    type="button"
                                                    className="btn btn-primary btn-icon-sm btn-edit au-target"/>
                                        </span> : null}
                                    </div>
                                ...
                                {props.data.owners[props.owner_id].user_id ? null :
                                    <div className="form-group">
                                        <div className="input-group">
                                ...
                                <div className="alert alert-success" role="alert">12321321</div>

                            </div>
                            <div className="modal-footer">
                                <button onClick={() => resetChanges()} type="button" className="btn btn-default"
                                        data-dismiss="modal">Отменить
                                </button>
                                <button type="button"
                                        disabled={props.data.owners[props.owner_id].user_id?!this.state.email||!this.state.phone:!this.state.email||!this.state.phone||!this.state.login}
                                        onClick={()=>{props.data.owners[props.owner_id].user_id? updateUser():createUser()}}
                                        className="btn btn-primary">{props.data.owners[props.owner_id].user_id ? "Восстановить" : "Создать"}</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="col-lg-4">
                            ...
                            <div className="row">
                                <div className="col-sm-6">
                                    <div
                                        className={`form-group ${ this.state.errors.document['ownershipType'] ? 'has-error' : null }`}>
                                        <select className="form-control"
                                                name="ownershipType"
                                                value={this.props.data.ownershipTypes[this.state.editor.ownershipType] ? this.state.editor.ownershipType : 'private'}
                                                onChange={setEditorValue}
                                        >
                                            {Object.keys(this.props.data.ownershipTypes).map(key => <option value={key}
                                                                                                            key={key}> {this.props.data.ownershipTypes[key]} </option>)}
                                        </select>
                                        <label className="control-label">
                                ...
                                <div className="col-sm-6">
                                    <div
                                        className={`form-group ${ this.state.errors.document['ownershipStartDate'] ? 'has-error' : null }`}>
                                        <input type="date"
                        ...
                        <div className="col-lg-8">
                            ...
                            <div className="row">
                                <div className="col-sm-3">
                                    <div
                                        className={`form-group ${ this.state.errors.document['documentType'] ? 'has-error' : null }`}>
                                        <input type="text"
                                               className="form-control"
                                               name="documentType"
                                ...
                                <div className="col-sm-3">
                                    <div
                                        className={`form-group ${ this.state.errors.document['documentNumber'] ? 'has-error' : null }`}>
                                        <input type="text"
                                               className="form-control"
                                               name="documentNumber"
                                ...
                                <div className="col-sm-3">
                                    <div
                                        className={`form-group ${ this.state.errors.document['documentCreated'] ? 'has-error' : null }`}>
                                        <input type="date"
                                               className="form-control"
                                               max={DateToTextValueConverter.getData(Date.now(), 'YMD', {date: true})}
                        ...
                        <tr>
                            <td colSpan={5}>
                                {this.state.editor.owners.length && this.state.editor.total_parts < 1
                                    ? <div className="text-right text-danger">Сумма долей собственников меньше
                                        единицы.</div>
                                    : this.state.editor.owners.length && this.state.editor.total_parts > 1
                                        ? <div className="text-right text-danger">Сумма долей собственников больше
                                            единицы.</div>
                                        : null}
                            </td>
                            <td colSpan={4}>
                                <button type="button" className="btn btn-primary btn-add btn-icon btn-block btn-raised"
                                        title="Отменить" onClick={addOwner}>Добавить
                                </button>
                            </td>
                        </tr>
                        </tbody>
                    ...
                    <div className="btn-group-flex pull-right">
                        {this.state.isSaving
                            ? <span className="form-inline"
                                    style={{padding: "4px 10px 0 0", opacity: 0.7, fontWeight: 'bold'}}><i
                                className="material-icons rotated">autorenew</i> СОХРАНЯЕМ</span>
                            : null}
                        &nbsp;
                        <button type="button" className="btn btn-default btn-clear btn-icon" title="Отменить"
                                onClick={() => {
                                    if (!this.state.isSaving) {
                                        this.props.disableEditor('owners')
                                    }
                                }}>Отменить
                        </button>
                        &nbsp;
                        <button type="button"
                                className={`btn btn-primary btn-save btn-icon ${ this.state.isModified ? 'btn-raised' : '' }`}
                                title="Сохранить" onClick={saveOwners}>Сохранить
                        </button>
                    </div>
                            ...
                            <div className="alert-content">
                                    <span className="alert-title">Заполните правильно:&nbsp;
                                        {Object.keys(this.state.errors.document).map((key, index) => (
                                            <strong
                                                key={key}>{this.state.errors.document[key]}{index < Object.keys(this.state.errors.document).length - 1 ? ', ' : ''}</strong>
                                        ))}
                                    </span>
                            </div>
                        </div>
                        : <div className="alert alert-icon alert-warning" style={{marginBottom: 0}}>
                            <div className="alert-content"><strong className="alert-title">Предыдущие документы будут
                                помечены как «недействительные»</strong></div>
                        </div>
                }
    ...
    render() {
        ...
        return (
            <tr>
                ...
                <td>
                    {!this.props.owner.forceInput
                        ? <button type="button" className="btn btn-primary btn-icon btn-edit btn-block" tooltip="Ввести"
                                  title=""
                                  onClick={() => this.props.createOwnerFromSearch(this.props.index, this.state.searchString || '')}>Создать</button>
                        : <button type="button" className="btn btn-primary btn-icon btn-filter btn-block"
                                  tooltip="Выбрать" title=""
                                  onClick={() => this.props.toggleOwnerRowForceInput(this.props.index, false)}>Выбрать</button>
                    }
                </td>
                {!this.props.owner.forceInput
                    ...
                    : <td className="td-form" style={{padding: '4px'}}>
                        <div
                            className={`form-group ${ !!this.props.errors && this.props.errors.indexOf('birth_date') >= 0 ? 'has-error' : '' }`}>
                            <input type="date"
                                   max={DateToTextValueConverter.getData(Date.now(), 'YMD', {date: true})}
                ...
                <td>
                    <button type="button" className="btn btn-default btn-icon-sm btn-clear" title="Отменить"
                            onClick={() => this.props.deleteOwnerRow(this.props.index)}/>
                </td>
            </tr>
        );
    }
...
const RoomOwnersEditorInputs = (props) => {
    return (
        [<td className="td-form" key="last" style={{padding: '4px'}}>
            <div
                className={`form-group ${ !!props.errors && props.errors.indexOf('lastName') >= 0 ? 'has-error' : '' }`}>
            ...
            <td className="td-form" key="first" style={{padding: '4px'}}>
                <div
                    className={`form-group ${ !!props.errors && props.errors.indexOf('firstName') >= 0 ? 'has-error' : '' }`}>
                    <input className="form-control"
            ...
            <td className="td-form" key="middle" style={{padding: '4px'}}>
                <div
                    className={`form-group ${ !!props.errors && props.errors.indexOf('middleName') >= 0 ? 'has-error' : '' }`}>
                    <input className="form-control"
