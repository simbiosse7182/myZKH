                    ...
                    <div className="panel-heading panel-toggle-sm">
                        ...
                        <div className="pull-right">
                            {!this.state.is_editing
                                ? <button type="button"
                                          className="btn btn-icon btn-primary btn-raised btn-edit pull-right"
                                ...
                                : <button type="button" className="btn btn-default btn-clear btn-icon" title="Отменить"
                                          onClick={() => this.toggleEdit()}>Закрыть</button>
                            }
                        </div>
                    </div>
                </div>
                <div className="nested-table-fix">
                    <table className="nested-table hovered">
                        ...
                        {this.state.is_editing || true
                            ? <tr className={`nested ${this.state.is_creating ? 'opened' : ''}`}
        ...
        return [
            <tr className={`nested ${this.props.isOpened ? 'opened' : ''}`} key={1} onClick={this.toggleOpen}>
                <td className="text-left text-uppercase">
                    {this.title}{` `}
                    <span
                        className="text-muted">( {Object.keys(this.props.services).filter(el => this.props.services[el].is_active).length || 0} шт. )</span>
                </td>
                <td>
                    {
                    ...
                    <td colSpan="3">
                        <table className="nested-table">
                            ...
                            <tr>
                                ...
                                {this.props.assigned && this.props.room && this.props.entrances ?
                                    <th>Назначение</th> : null}
            ...
            <td className="text-left">
                ...
                {service.end_date
                    ? (+Date.parse(service.end_date) - +Date.now()) < 86400000 * 3
                        ? <span
                            className="pull-right text-danger">(до {dateFormat(Date.parse(service.end_date), 'yyyy.mm.dd')})</span>
                        : <span
                            className="pull-right text-muted">(до {dateFormat(Date.parse(service.end_date), 'yyyy.mm.dd')})</span>
                    : null
                }
            </td>
            {   // НАЗНАЧЕНИЕ
            ...
            <td className={`${ props.isEditing && service.is_editable ? '' : 'td-btn-fix'}`}>
                {
                    props.isEditing
                        ...
                        ? service.is_editable
                        ? <div className="btn btn-icon btn-edit btn-primary btn-block"
                               onClick={() => props.toggleItemEdit(service.service_id)}>Изменить</div>
                        : ''
                        // Стоимость
                        : !!service_types[service.service_type_id]
                        ? service_types[service.service_type_id].type === 1 // если разовая услуга
...
class RoomsServiceCreator extends React.Component {
    ...
    render() {
        ...
        let setStateValue = (name, value) => {
            ...
            _state[name] = value;
            if (name === 'service_type_id' || name === 'connection_type') {
                _state['connection_details'] = [];
            }
            ...
            if (+this.state.service_type_id && this.props.serviceTypes[+this.state.service_type_id]) { // если услуга задана
                ...
                if (this.props.type === 'assigned' && +service_type.assignment_type !== 3) {
                    ...
                    if ((+this.state.connection_type === 2 || +this.state.connection_type === 3)
                        && +this.state.connection_details.length <= 0) {
                        errors.service['connection_details'] = 'объекты назначения';
                    }
                }
            ...
            <div className="panel">
                <div className="panel-body text-left" style={{marginTop: '1px'}}>
                    ...
                    <div className="row">
                        <div className="col-sm-3">
                            <div
                                className={`form-group ${ this.state.errors.service['service_type_id'] ? 'has-error' : null }`}>
                        ...
                        {   // Единица расчёта. Только для периодических услуг.
                            (service_types[this.state.service_type_id] && +service_types[this.state.service_type_id].type !== 1) || this.props.type === 'assigned'
                                ? <div className="col-sm-3">
                                    <div
                                        className={`form-group ${ this.state.errors.service['calculation_unit'] ? 'has-error' : null }`}>
                                        <select className="form-control"
                        ...
                        <div className="col-sm-3">
                            <div
                                className={`form-group ${ this.state.errors.service['end_date'] ? 'has-error' : null }`}>
                                <input type="date"
                                       className="form-control"
                                       name="end_date"
                        ...
                        {   // Объем. Только для разовых.
                            service_types[this.state.service_type_id] && +service_types[this.state.service_type_id].type === 1
                                ? <div className="col-sm-3">
                                    <div
                                        className={`form-group ${ this.state.errors.service['value'] ? 'has-error' : null }`}>
                                        <input type="number"
                                               className="form-control"
                                               name="value"
                ...
                <div className="panel-body">
                    ...
                    {this.state.service_id
                        ? <button type="button"
                                  className="btn btn-default btn-clear btn-icon btn-danger btn-raised pull-left"
                                  title="Удалить" onClick={() => {
                            if (!this.state.isSaving) {
                                dropService()
                            }
                        }}>Закрыть Услугу</button>
                        : null
                    }
                    ...
                    <div className="btn-group-flex pull-right">
                        ...
                        &nbsp;
                        <button type="button" className={`btn btn-primary btn-save btn-icon btn-raised`}
                                title="Сохранить" onClick={saveServices}>Сохранить
                        </button>
                    </div>
                            ...
                            <div className="alert-content">
                                        <span className="alert-title">Заполните правильно:&nbsp;
                                            {Object.keys(this.state.errors.service).map((key, index) => (
                                                <strong
                                                    key={key}>{this.state.errors.service[key]}{index < Object.keys(this.state.errors.service).length - 1 ? ', ' : ''}</strong>
                                            ))}
                                        </span>
                            </div>
                        </div>
                        : this.props.type === 'house_common'
                            ? <div>
                                <div className="alert alert-info" style={{marginBottom: 0}}>
                                    <div className="alert-content"><strong className="alert-title">Изменения будут
                                        применены для всех квартир этого дома</strong></div>
                                </div>
                            </div>
                            : this.props.type === 'house_common'
                                ? <div>
                                    <div className="alert alert-info" style={{marginBottom: 0}}>
                                        <div className="alert-content"><strong className="alert-title">Изменения будут
                                            применены для всех квартир в объектах назначения</strong></div>
                                    </div>
                                </div>
                                : null
                }
            ...
            <div>
...

                {
                    Object.keys(this.state.service_counters)
                        ...
                        .map(service_id =>
                            <RoomCounterServiceRow
                                room={this.props.room}
                                service={this.props.data.services[service_id]}
                                data={this.props.data}
    ...
    render() {
        return (
            <div className="panel panel-default">
                ...
                {
                    this.props.counters.map(counter_id =>
                        <RoomCounterRow
                            room={this.props.room}
                            key={counter_id}
...
class RoomCounterRow extends React.Component {
    ...
    render() {
        const counter_id = this.props.counter.counter_id;
        const service_values = this.props.room.service_values.filter(el => el.counter_id === counter_id).sort((a, b) => new Date(a.created) < new Date(b.created) ? 1 : -1);
        let lastInputValue = service_values[0].value;
        let lastInputDate = service_values[0].created;
                    ...
                    <div className="col-sm-3">
                        <div className="bem-form-group__inner-form-group form-group form-group-readonly">
                            <div className="input-group">
                                    <span className="form-control">
                                        <label className="control-label">
                                         <span>Показание от {dateformat(lastInputDate, "dd.mm.yyyy")}</span>
                                        </label>
                                        {lastInputValue}
...
{/*service_values.map((service_value, index) => <tr key={index}>*/
}
{/*<td>{service_value.value}</td>*/
}
{/*<td> {dateformat(service_value.created, "dd.mm.yyyy")}</td>*/
}
{/*</tr>*/
}
{/*)}*/
